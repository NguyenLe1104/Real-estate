...
const houseRecommendationKey = (userId: number) => `recommendations:houses:${userId}`;
const landRecommendationKey = (userId: number) => `recommendations:lands:${userId}`;
...
export class RecommendationService {
    ...
    async getAIRecommendations(userId: number, limit = 10) {
        ...
        const interactedHouseIds = new Set(interactions.filter(i => i.type === 'house').map(i => i.id));
        const interactedLandIds = new Set(interactions.filter(i => i.type === 'land').map(i => i.id));

        // 4. Fallback: no behavior → popular/recent mix
        if (interactions.length === 0) {
            const fallback = await this.getPopularMixed(limit);
            await this.redis.set(cacheKey, fallback, RECOMMENDATION_TTL).catch(() => { });
            return fallback;
        }
        ...
        const allInteractedProperties = await this.fetchInteractedProperties(interactions);
        const profile = this.buildUserProfile(
            allInteractedProperties.map(p => ({ ...p, weight: interactionMap.get(`${p.type}:${p.id}`)?.weight || 1 })),
        );
        ...
        for (const house of dbHouses) {
            ...
            const finalScore = weightEmbedding * embeddingScore + weightRule * ruleScore;

            // Freshness boost
            const daysSinceCreated = (Date.now() - new Date(house.createdAt).getTime()) / (1000 * 60 * 60 * 24);
        ...
        for (const land of dbLands) {
            ...
            const { score: ruleScore, reasons } = this.calculateLandScore(land, profile, landTypeCounts);
            const embeddingScore = embeddingScoreMap.get(`land:${land.id}`) || 0;
            const finalScore = weightEmbedding * embeddingScore + weightRule * ruleScore;

            const daysSinceCreated = (Date.now() - new Date(land.createdAt).getTime()) / (1000 * 60 * 60 * 24);
            let boostedScore = finalScore;
            if (daysSinceCreated < 7) {
                boostedScore += 0.03;
                reasons.push('Mới đăng');
        ...
        for (const vc of vectorCandidates) {
            ...
            if (hybridScored.some(h => h.id === sourceId && h.type === source)) continue;
        ...
        const topHouseIds = diversified.filter(d => d.type === 'house').map(d => d.id);
        const topLandIds = diversified.filter(d => d.type === 'land').map(d => d.id);
        ...
        const result = diversified.map(item => {
            const property = item.type === 'house' ? houseMap.get(item.id) : landMap.get(item.id);
        ...
        this.logger.debug(`Generated ${result.length} AI hybrid recommendations for user ${userId}`);
    ...
    private async buildUserVector(userId: number, interactions: WeightedInteraction[]): Promise<number[] | null> {
        // Check cached user vector
        const vecCacheKey = userVectorKey(userId);
        const cachedVector = await this.redis.get<number[]>(vecCacheKey).catch(() => null);
        ...
        try {
            ...
            this.logger.warn(`Failed to fetch point vectors from Qdrant: ${this.stringifyError(error)}`);
        }
        ...
        if (pointVectors.length === 0) {
            ...
            if (fallbackVector) {
                await this.redis.set(vecCacheKey, fallbackVector, EMBEDDING_CACHE_TTL).catch(() => { });
            }
        ...
        await this.redis.set(vecCacheKey, avgVector, EMBEDDING_CACHE_TTL).catch(() => { });
        this.logger.debug(`Built user vector from ${pointVectors.length} embeddings (${interactions.length} interactions)`);

        return avgVector;
    }

    private async buildUserVectorFromText(interactions: WeightedInteraction[]): Promise<number[] | null> {
        // Fallback: build a text summary from user interactions and embed it
        const topInteractions = interactions
            .sort((a, b) => b.weight - a.weight)
            .slice(0, 10);

        const houseIds = topInteractions.filter(i => i.type === 'house').map(i => i.id);
        const landIds = topInteractions.filter(i => i.type === 'land').map(i => i.id);

        const [houses, lands] = await Promise.all([
            houseIds.length > 0
                ? this.prisma.house.findMany({
                    where: { id: { in: houseIds } },
                    select: { title: true, city: true, district: true, price: true, area: true },
                })
                : Promise.resolve([]),
            landIds.length > 0
                ? this.prisma.land.findMany({
        ...
        const parts = [
            ...houses.map(h => `Nha: ${h.title}, ${h.district} ${h.city}, gia ${h.price}, dt ${h.area}`),
            ...lands.map(l => `Dat: ${l.title}, ${l.district} ${l.city}, gia ${l.price}, dt ${l.area}`),
        ];
        ...
        try {
            ...
            this.logger.warn(`Fallback embedding failed: ${this.stringifyError(error)}`);
        }
    ...
    ): Promise<Array<{ id: number; type: 'house' | 'land'; price: any; city: string | null; district: string | null; area: number | null; categoryId: number | null; landType?: string | null }>> {
        const houseIds = interactions.filter(i => i.type === 'house').map(i => i.id);
        const landIds = interactions.filter(i => i.type === 'land').map(i => i.id);

        const [houses, lands] = await Promise.all([
            houseIds.length > 0
                ? this.prisma.house.findMany({
                    where: { id: { in: houseIds } },
                    select: { id: true, price: true, city: true, district: true, area: true, categoryId: true },
                })
                : Promise.resolve([]),
            landIds.length > 0
                ? this.prisma.land.findMany({
                    where: { id: { in: landIds } },
                    select: { id: true, price: true, city: true, district: true, area: true, categoryId: true, landType: true },
                })
                : Promise.resolve([]),
        ]);
    ...
    async trackBehavior(userId: number, action: string, houseId?: number, landId?: number) {
        ...
        await Promise.all([
            houseId ? this.redis.del(houseRecommendationKey(userId)).catch(() => { }) : Promise.resolve(),
            landId ? this.redis.del(landRecommendationKey(userId)).catch(() => { }) : Promise.resolve(),
    ...
    async getHouseRecommendations(userId: number, limit = 5) {
        ...
        const behaviors = await this.prisma.userBehavior.findMany({
            ...
            include: {
                house: {
                    select: {
                        id: true, price: true, city: true, district: true,
                        ward: true, area: true, direction: true, categoryId: true,
        ...
        const interactedItems = [
            ...behaviors.filter(b => b.house).map(b => ({
                ...b.house!,
                weight: b.action === 'save' ? 3 : b.action === 'click' ? 2 : 1,
            })),
            ...favorites.filter(f => f.house).map(f => ({
                ...f.house!,
                weight: 3,
            })),
        ];
        ...
        if (interactedItems.length === 0) {
            ...
            await this.redis.set(cacheKey, popular, RECOMMENDATION_TTL).catch(() => { });
            return popular;
        }
        ...
        await this.redis.set(cacheKey, result, RECOMMENDATION_TTL).catch(() => { });
        this.logger.debug(`Generated ${result.length} house recommendations for user ${userId}`);
    ...
    async getLandRecommendations(userId: number, limit = 5) {
            ...
            include: {
                land: {
                    select: {
                        id: true, price: true, city: true, district: true,
                        ward: true, area: true, direction: true, categoryId: true,
                        frontWidth: true, landLength: true, landType: true,
                    },
                },
            },
            orderBy: { createdAt: 'desc' },
        ...
        // Build user profile from behavior
        const interactedItems = [
            ...behaviors.filter(b => b.land).map(b => ({
                ...b.land!,
                weight: b.action === 'save' ? 3 : b.action === 'click' ? 2 : 1,
            })),
            ...favorites.filter(f => f.land).map(f => ({
                ...f.land!,
                weight: 3,
            })),
        ];
        ...
        interactedItems.forEach(item => {
            ...
            if (land.landType) {
                landTypeCounts[land.landType] = (landTypeCounts[land.landType] || 0) + item.weight;
            }
        });
        ...
        const scored: ScoredProperty[] = candidates.map(land => {
            const { score, reasons } = this.calculateLandScore(land, profile, landTypeCounts);
            return {
                id: land.id,
                score: Math.round(score * 100) / 100,
                reason: reasons.join(', '),
        ...
        await this.redis.set(cacheKey, result, RECOMMENDATION_TTL).catch(() => { });
        this.logger.debug(`Generated ${result.length} land recommendations for user ${userId}`);
    ...
    private buildUserProfile(items: Array<any & { weight: number }>): UserProfile {
        const totalWeight = items.reduce((sum, h) => sum + h.weight, 0);

        // Weighted average price
        const prices = items.filter(h => h.price).map(h => ({
            value: Number(h.price),
            weight: h.weight,
        }));
        const avgPrice = prices.length > 0
            ? prices.reduce((sum, p) => sum + p.value * p.weight, 0) / prices.reduce((sum, p) => sum + p.weight, 0)
            : 0;
        ...
        items.forEach(h => {
            if (h.categoryId) {
                categoryCounts[h.categoryId] = (categoryCounts[h.categoryId] || 0) + h.weight;
            }
        });

        // Average area
        const areas = items.filter(h => h.area).map(h => ({
            value: Number(h.area),
            weight: h.weight,
        }));
        const avgArea = areas.length > 0
            ? areas.reduce((sum, a) => sum + a.value * a.weight, 0) / areas.reduce((sum, a) => sum + a.weight, 0)
            : 0;
    ...
    private calculateScore(
        ...
        if (profile.avgPrice > 0 && property.price) {
            ...
            const priceDiff = Math.abs(propPrice - profile.avgPrice) / profile.avgPrice;
        ...
        if (property.city && property.district) {
            ...
            const maxLocationWeight = Math.max(...Object.values(profile.locationCounts), 1);
        ...
        if (property.categoryId && profile.categoryCounts[property.categoryId]) {
            const maxCatWeight = Math.max(...Object.values(profile.categoryCounts), 1);
            similarityScore += (profile.categoryCounts[property.categoryId] / maxCatWeight) * 0.4;
            similarityCount++;
        }

        // Area similarity
        if (profile.avgArea > 0 && property.area) {
            const areaDiff = Math.abs(Number(property.area) - profile.avgArea) / profile.avgArea;
    ...
    private async getPopularHouses(limit: number) {
        ...
        if (popularIds.length > 0) {
            const houses = await this.prisma.house.findMany({
                where: { id: { in: popularIds.map(p => p.houseId!).filter(Boolean) }, status: 1 },
                include: {
                    images: { select: { id: true, url: true } },
                    category: true,
                    employee: {
    ...
    private async getPopularLands(limit: number) {
        ...
        if (popularIds.length > 0) {
            const lands = await this.prisma.land.findMany({
                where: { id: { in: popularIds.map(p => p.landId!).filter(Boolean) }, status: 1 },
                include: {
                    images: { select: { id: true, url: true } },
                    category: true,
                    employee: {
