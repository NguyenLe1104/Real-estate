...
const VIP_TIER_RULES: Record<number, { durationDays: number; badge: string; features: Omit<FeatureFlags, 'badge'> }> = {
    1: {
        durationDays: 3,
        badge: 'VIP Basic',
...
export class VipPackageService {
    ...
    private buildFeatureSignature(durationDays: number, features: FeatureFlags): string {
        return [
            durationDays,
    ...
    private normalizeFeatures(raw?: string): string | null | undefined {
        ...
        if (obj.badge !== undefined) {
            ...
            if (badge && !/^(?=.{1,30}$)[\p{L}\p{N}\s-]+$/u.test(badge)) {
                throw new BadRequestException('Badge chỉ cho phép chữ, số, khoảng trắng và dấu gạch ngang');
            }
    ...
    private async ensureUniqueActivePriority(priorityLevel: number, excludeId?: number) {
        ...
        if (existed) {
            throw new BadRequestException('Mức độ ưu tiên đã tồn tại, vui lòng chọn mức khác');
        }
    }
    ...
    private async enforceTierRules(input: {
        ...
        if (!VIP_TIER_RULES[input.priorityLevel]) {
            throw new BadRequestException('Chỉ cho phép mức ưu tiên từ 1 đến 4 theo bộ gói VIP chuẩn');
        }
        ...
        if (input.durationDays !== tier.durationDays) {
            throw new BadRequestException(`Mức ưu tiên ${input.priorityLevel} phải có thời hạn ${tier.durationDays} ngày`);
        }

        if (flags.badge !== tier.badge) {
            throw new BadRequestException(`Mức ưu tiên ${input.priorityLevel} phải dùng badge ${tier.badge}`);
        }
        ...
        if (
            ...
            throw new BadRequestException('Tính năng gói không đúng mẫu phân tầng đã quy định');
        }
        ...
        const currentSignature = this.buildFeatureSignature(input.durationDays, flags);
        const duplicateByDurationAndFeature = activePackages.find((pkg) => {
            const pkgSignature = this.buildFeatureSignature(pkg.durationDays, this.parseFeatures(pkg.features));
            return pkgSignature === currentSignature;
        });

        if (duplicateByDurationAndFeature) {
            throw new BadRequestException('Đã tồn tại gói active có cùng thời hạn và bộ tính năng');
        }
        ...
        for (let i = 1; i < allForPriceCheck.length; i += 1) {
            if (allForPriceCheck[i].price <= allForPriceCheck[i - 1].price) {
                throw new BadRequestException('Giá gói cao hơn phải lớn hơn giá gói có mức ưu tiên thấp hơn');
            }
        }
    }
    ...
    async updatePackage(id: number, dto: UpdateVipPackageDto) {
        ...
        await this.enforceTierRules({
            ...
            features: normalizedFeatures !== undefined ? normalizedFeatures : vipPackage.features,
        });
        ...
        const updated = await this.prisma.vipPackage.update({
            ...
            data: {
                ...dto,
                ...(normalizedFeatures !== undefined ? { features: normalizedFeatures } : {}),
            },
        });
