...
export class AppointmentAutoAssignConsumerController {
    private readonly logger = new Logger(AppointmentAutoAssignConsumerController.name);
    ...
    async handleAutoAssign(@Payload() payload: AutoAssignPayload): Promise<void> {
        ...
        this.logger.log(`[RabbitMQ] Nhận job auto-assign cho appointment #${payload.appointmentId}`);
        try {
            await this.appointmentService.autoAssign(payload.appointmentId);
            this.logger.log(`[RabbitMQ] Auto-assign hoàn tất cho appointment #${payload.appointmentId}`);
        } catch (error) {
            this.logger.error(
