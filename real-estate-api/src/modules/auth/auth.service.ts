...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    ...
    return {
      message:
        'Mã OTP đã được gửi đến email. Vui lòng xác thực để hoàn tất đăng ký.',
  ...
  async confirmRegister(dto: ConfirmRegisterDto) {
    ...
    await this.prisma.customer.create({
      data: {
        userId: newUser.id,
        code: `KH${newUser.id.toString().padStart(3, '0')}`,
      },
    });
  ...
  async loginGoogle(idToken: string) {
    ...
    if (!payload)
      throw new BadRequestException('Dữ liệu token Google không hợp lệ');
    ...
    await this.prisma.customer.upsert({
      ...
      create: {
        userId: user.id,
        code: `KH${user.id.toString().padStart(3, '0')}`,
      },
      update: {},
    });
    ...
    const refreshExpiresMs = ms(
      (this.configService.get('JWT_REFRESH_EXPIRES') || '7d') as ms.StringValue,
    );
    await this.prisma.refreshToken.create({
      data: {
        token: refreshToken,
