...
export class EmployeeService {
  ...
  async create(dto: CreateEmployeeDto) {
    ...
    const existCode = await this.prisma.employee.findUnique({
      where: { code: dto.code },
    });
    if (dto.startDate) {
  ...
  async update(id: number, dto: UpdateEmployeeDto) {
    ...
    if (dto.code) {
      const existedCode = await this.prisma.employee.findUnique({
        where: { code: dto.code },
      });
    ...
    const updatedEmployee = await this.prisma.employee.update({
      ...
      data: {
        ...
        ...(dto.city !== undefined && {
          city: this.normalizeOptional(dto.city) ?? null,
        }),
        ...((dto.ward !== undefined || dto.district !== undefined) && {
          ward: this.normalizeOptional(dto.ward ?? dto.district) ?? null,
        }),
