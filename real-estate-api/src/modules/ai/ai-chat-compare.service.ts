...
export class AiChatCompareService {
    ...
    extractIdsFromHistory(memory: ChatTurn[]): number[] {
        ...
        for (const turn of assistantTurns) {
            ...
            const urlPatterns = [...text.matchAll(/\/(?:houses|lands|nha|dat)\/(\d+)/gi)];
            const sourceIdPatterns = [...text.matchAll(/"sourceId"\s*:\s*(\d+)/g)];

            for (const match of [...idPatterns, ...urlPatterns, ...sourceIdPatterns]) {
     ...
     */
    async findIdByDescription(description: string, excludeId?: number): Promise<number | null> {
        const stopWords = new Set([
            'nha', 'dat', 'can', 'tin', 'ban', 'cho', 'thue', 'mua', 'o', 'tai',
            'voi', 'va', 'de', 'la', 'duong', 'phuong', 'quan', 'tp', 'thanh',
            'pho', 'thi', 'xa', 'huyen', 'so', 'mat', 'tien', 'hem', 'ngo',
            'biet', 'thu', 'can', 'ho', 'chung', 'cu',
        ]);
     ...
     */
    async findByTextInDb(description: string, excludeId?: number, precomputedTokens?: string[]): Promise<number | null> {
        let tokens: string[];
        ...
        const scoreRecord = (r: {
            ...
            const titleWords = new Set(titleNorm.split(/\s+/).filter((w) => w.length > 0));
            const allWords = new Set(fullNorm.split(/\s+/).filter((w) => w.length > 0));
        ...
        const getBestMatch = (
            houses: Array<{ id: number; title: string | null; street: string | null; ward: string | null; district: string | null; city: string | null }>,
            lands: Array<{ id: number; title: string | null; street: string | null; ward: string | null; district: string | null; city: string | null }>,
        ...
        try {
            ...
            this.logger.warn(`findByTextInDb failed for "${description}": ${this.stringifyError(error)}`);
            return null;
        }
    }
    ...
    async buildCompareAnswer(ids: number[]): Promise<{
        ...
        if (found.length < 2) {
            return {
                answer: 'Mình chưa tìm thấy đủ bất động sản để so sánh. Bạn có thể mở lại 2 tin cần so sánh hoặc gửi link chi tiết của từng tin.',
                sources: [],
...
<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;margin:12px 0;max-width:100%;">
    ...
    private toNumber(value: unknown): number {
        ...
        if (value && typeof value === 'object') {
            const numericLike = value as { toNumber?: () => number; toString?: () => string; valueOf?: () => unknown };
