...
const Gallery: React.FC<GalleryProps> = ({ images, title }) => {
    ...
    /* ── Special layout for exactly 3 images: 1 big left + 2 stacked right ── */
    if (images.length === 3) {
        return (
            <>
                <div className="mb-10">
    ...
    /* ── Layout: 1 big left + thumbnails right (Airbnb style, 4+ images) ── */
    const thumbCount = Math.min(images.length - 1, 4);
    const thumbIndices = Array.from({ length: thumbCount }, (_, i) => i + 1);
    const extraCount = images.length - 5; // images beyond the displayed 5
    ...
    return (
        <>
            <div className="mb-10">
                        ...
                        {thumbIndices.map((imgIdx, gridPos) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {/* "+N more" badge on last visible thumb */}
                                    {isLastThumb && (
                                        <div
                ...
                {/* "Xem tất cả ảnh" pill button — bottom right */}
                {images.length > 1 && (
                    <div className="flex justify-end mt-2">
                        <button
                            onClick={() => openLightbox(0)}
