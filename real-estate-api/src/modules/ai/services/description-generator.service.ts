...
export class DescriptionGeneratorService {
    ...
    private readonly llmProvider = String(process.env.LLM_PROVIDER || '').toLowerCase();

    private readonly geminiApiKey = process.env.GEMINI_API_KEY || '';
    private readonly geminiApiUrl = process.env.GEMINI_API_URL || 'https://generativelanguage.googleapis.com/v1beta';
    private readonly geminiModelPrimary = process.env.GEMINI_MODEL_PRIMARY || 'gemini-1.5-flash';
    private readonly geminiModelFallback1 = process.env.GEMINI_MODEL_FALLBACK_1 || 'gemini-1.5-pro';
    private readonly geminiModelFallback2 = process.env.GEMINI_MODEL_FALLBACK_2 || 'gemini-1.5-flash-8b';
    private readonly geminiTimeoutMs = Number(process.env.GEMINI_TIMEOUT_MS || 15000);
    private readonly geminiMaxRetries = Number(process.env.GEMINI_MAX_RETRIES || 2);
    private readonly geminiRetryBaseMs = Number(process.env.GEMINI_RETRY_BASE_MS || 1200);
    private readonly geminiThinkingBudget = Number(process.env.GEMINI_THINKING_BUDGET || 0);

    private readonly postTypeIntentMap: Record<string, PostTypeIntent> = {
        SELL_HOUSE: {
            label: 'Bán nhà',
            objective: 'Viết bài đăng bán nhà thuyết phục, tập trung giá trị tài sản và tính thanh khoản.',
            requiredSections: 'TỔNG QUAN, GIÁ, TIỆN ÍCH, THÔNG SỐ, VỊ TRÍ, PHÁP LÝ',
            extraRules: [
                'Nhấn mạnh điểm nổi bật giúp chốt giao dịch nhanh.',
            ],
        },
        SELL_LAND: {
            label: 'Bán đất',
            objective: 'Viết bài đăng bán đất rõ ràng, nhấn mạnh tiềm năng tăng giá và pháp lý.',
        ...
        RENT_HOUSE: {
            ...
            objective: 'Viết bài cho thuê rõ điều kiện thuê, ưu tiên tiện ích sống và mức giá.',
            requiredSections: 'TỔNG QUAN, GIÁ THUÊ, TIỆN ÍCH, THÔNG SỐ, VỊ TRÍ, ĐIỀU KIỆN THUÊ',
            extraRules: [
        ...
        RENT_LAND: {
            ...
            objective: 'Viết bài cho thuê đất/mặt bằng phục vụ kinh doanh hoặc lưu trữ.',
            requiredSections: 'TỔNG QUAN, GIÁ THUÊ, THÔNG SỐ, VỊ TRÍ, MỤC ĐÍCH PHÙ HỢP, ĐIỀU KIỆN THUÊ',
            extraRules: [
                'Không dùng văn phong bán đứt tài sản.',
            ],
        },
        NEED_BUY: {
            label: 'Cần mua',
            objective: 'Viết bài đăng nhu cầu tìm mua bất động sản từ góc nhìn người mua.',
            requiredSections: 'TỔNG QUAN NHU CẦU, NGÂN SÁCH, TIÊU CHÍ BẤT ĐỘNG SẢN, KHU VỰC MONG MUỐN, THÔNG TIN LIÊN HỆ',
            extraRules: [
        ...
        NEED_RENT: {
            label: 'Cần thuê',
            objective: 'Viết bài đăng nhu cầu tìm thuê nhà/căn hộ/phòng trọ/đất từ góc nhìn người thuê.',
            requiredSections: 'TỔNG QUAN NHU CẦU, NGÂN SÁCH THUÊ, TIÊU CHÍ, KHU VỰC MONG MUỐN, THÔNG TIN LIÊN HỆ',
            extraRules: [
        ...
        NEWS: {
            label: 'Tin tức',
            objective: 'Viết bài tin tức bất động sản trung lập, thông tin rõ ràng, không mang tính chốt sale.',
            requiredSections: 'TỔNG QUAN, DIỄN BIẾN CHÍNH, TÁC ĐỘNG THỊ TRƯỜNG, GỢI Ý CHO NGƯỜI ĐỌC',
            extraRules: [
        ...
        PROMOTION: {
            label: 'Khuyến mãi',
            objective: 'Viết bài khuyến mãi rõ điều kiện áp dụng, thời gian và mã ưu đãi.',
            requiredSections: 'TỔNG QUAN ƯU ĐÃI, NỘI DUNG KHUYẾN MÃI, THỜI GIAN ÁP DỤNG, ĐIỀU KIỆN, CÁCH NHẬN ƯU ĐÃI',
            extraRules: [
    ...
    async generateDescription(dto: GenerateDescriptionDto): Promise<{ description: string }> {
        ...
        const isNeedPost = normalizedPostType === 'NEED_BUY' || normalizedPostType === 'NEED_RENT';
        const isContentPost = normalizedPostType === 'NEWS' || normalizedPostType === 'PROMOTION';

        let promptTone = tone === 'polite'
            ? 'Lịch sự, chuyên nghiệp, rõ ràng và đáng tin cậy.'
            : 'Thân thiện, gần gũi, dễ đọc, dùng emoji vừa phải và đúng ngữ cảnh.';

        if (isNeedPost) {
            promptTone = tone === 'polite'
            ...
            promptTone = 'Lịch sự, chuyên nghiệp, nhấn mạnh thông tin quan trọng, trình bày mạch lạc.';
        }
        ...
        const details = [
            ...
            minArea ? `Diện tích tối thiểu mong muốn: ${this.formatArea(minArea)}` : null,
            maxArea ? `Diện tích tối đa mong muốn: ${this.formatArea(maxArea)}` : null,
        ...
        const postTypeRules = intent.extraRules.length > 0
...
`.trim();
        ...
        try {
            ...
            return {
                description: generatedText || 'Hiện không thể tạo mô tả. Vui lòng thử lại.'
            };
        } catch (error) {
            this.logger.error(`generateDescription error: ${this.stringifyError(error)}`);
            return {
                description: 'Đã có lỗi xảy ra khi gọi AI để tạo mô tả. Vui lòng thử lại sau.'
            };
        }
    }

    async generatePropertyDescription(question: string, conversation: ConversationState): Promise<string> {
    ...
    private async generateWithProviders(
        ...
        if (this.llmProvider && this.llmProvider !== 'gemini') {
            this.logger.warn(`LLM_PROVIDER=${this.llmProvider} is set. API-only mode expects gemini.`);
            return '';
        }
        ...
        for (const model of modelCandidates) {
            for (let attempt = 0; attempt <= this.geminiMaxRetries; attempt++) {
                try {
                    ...
                    const waitMs = this.extractRetryDelayMs(error) ?? this.geminiRetryBaseMs * (attempt + 1);
    ...
    private extractRetryDelayMs(error: unknown): number | null {
        ...
        const retryDelayRaw = error.response?.data?.error?.details
            ?.find((d: { '@type'?: string }) => d?.['@type'] === 'type.googleapis.com/google.rpc.RetryInfo')
            ?.retryDelay;
